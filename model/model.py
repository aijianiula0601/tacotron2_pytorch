...
class Encoder(nn.Module):
    ...
    def forward(self, x, input_lengths, input_max_len):
        ...
        outputs, _ = nn.utils.rnn.pad_packed_sequence(
            outputs, batch_first=True, total_length=input_max_len)
...
class Decoder(nn.Module):
    ...
    def forward(self, memory, decoder_inputs, input_max_len, memory_lengths):
        ...
        self.initialize_decoder_states(
            memory, mask=~get_mask_from_lengths(memory_lengths, input_max_len))
...
class Tacotron2(nn.Module):
    ...
    def parse_batch(self, batch):
        ...
        input_max_len = torch.max(input_lengths.data).item()
        mel_padded = to_var(mel_padded).float()
        gate_padded = to_var(gate_padded).float()
        output_lengths = to_var(output_lengths).long()
        output_max_len = torch.max(output_lengths.data).item()

        return (
            (text_padded, input_lengths, mel_padded, input_max_len, output_lengths, output_max_len),
            (mel_padded, gate_padded))

    def parse_output(self, outputs, output_max_len=None, output_lengths=None):
        if self.mask_padding and output_lengths is not None:
            mask = ~get_mask_from_lengths(output_lengths, output_max_len, True)  # (B, T)
    ...
    def forward(self, inputs):
        text_inputs, text_lengths, mels, input_max_len, output_lengths, output_max_len = inputs
        text_lengths, output_lengths = text_lengths.data, output_lengths.data

        embedded_inputs = self.embedding(text_inputs).transpose(1, 2)

        encoder_outputs = self.encoder(embedded_inputs, text_lengths, input_max_len)

        mel_outputs, gate_outputs, alignments = self.decoder(
            encoder_outputs, mels, input_max_len, memory_lengths=text_lengths)

        mel_outputs_postnet = self.postnet(mel_outputs)
        mel_outputs_postnet = mel_outputs + mel_outputs_postnet

        return self.parse_output(
            [mel_outputs, mel_outputs_postnet, gate_outputs, alignments], output_max_len,
            output_lengths)
